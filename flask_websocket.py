...
def patch_flask_websocket(app):
    def websocket_route(*args, **kwargs):
        if kwargs.get("websocket", False):
            def websocket_route_decorator(f):
                @wraps(f)
                def websocket_route_request(*args, **kwargs):
                    ws = Server(request.environ, {})
                    try:
                        code, msg = f(ws, *args, **kwargs)
                    except ConnectionClosed:
                        pass
                    try:
                        ws.close(reason=code, message=msg)
                    except:
                        pass
                    return []
                app._original_route(*args, **kwargs)(websocket_route_request)
        else:
            def websocket_route_decorator(f):
                @wraps(f)
                def route_request(*args, **kwargs):
                    return f(*args, **kwargs)
                app._original_route(*args, **kwargs)(route_request)
        return websocket_route_decorator
    app._original_route = app.route
